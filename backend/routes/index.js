...
// ✅ Change: Use a fallback to prevent the "must have a value" crash
...
router.post("/login", async (req, res) => {
  ...
  if (user) {
    bcrypt.compare(password, user.password, (err, isMatch) => {
      if (isMatch === true) {
        // ✅ Ensure we have a valid secret before signing
...
router.post("/createProject", async (req, res) => {
  ...
  if (user) {
    ...
    return res.json({ success: true, message: "New project is created successfully!", projectId: project._id });
  } else {
    return res.json({ success: false, message: "User not found" });
  }
});
